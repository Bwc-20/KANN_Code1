...
def get_validation_primary(cols, cnv_split):
    ...
    if cnv_split:     ### 现在来整理拷贝数数据，就是来看拷贝数扩增以及拷贝数减少的数据，使这些数据取整，相继归零或者归1
        ...
        data_type_list = ['mut', 'cnv_del', 'cnv_amp']      ## 最终就是输入这三类数据

    all_data = pd.concat(df_list, keys=data_type_list, join='inner', axis=1, )       ## 现在这个all_data就是输入的三种类型数据的一个融合！
    all_data = all_data.swaplevel(i=0, j=1, axis=1)

    # order the columns based on genes
    order = all_data.columns.levels[0]        ## 按照基因将融合后的组合数据进行排序
    all_data = all_data.reindex(columns=order, level=0)
    all_data.fillna(0, inplace=True)       ### 将这个矩阵中的空缺位置用0进行填充
    x = all_data.as_matrix()
    cols = all_data.columns
    rows = pd.DataFrame(index=all_data.index)
    y = np.zeros((x.shape[0],))
    return x, y, rows, cols      ## 现在返回的x:输入的三种类型数据的一个组合矩阵   y: 代表初始化的那个标签序列，就是当前这个患者他的癌症是早期还是晚期    rows, cols 就是这三种类型的组合数据分别按照行列拆分来看时的情况
...
### 这个是来整合数据的最终是要将那些癌症状态是转移性的那些患者样本他们的一些基因属性数据（突变与CNV）整合到一起形成矩阵，并将这个矩阵的一些信息返回回来
def get_validation_metastatic(cols, cnv_split):     ### 这个是获取癌症状态是转移性的那些患者样本
    ### 下面的这个样本是正常的健康的样本
    ...
    met500_samples = set(prostate_samples).difference(common_samples)         ### 现在这个met500_samples是返回集合的差集，就是在癌症样本集中但是不在正常的样本集中的数据
    ...
    valid_mut = pd.read_csv(join(validation_data_dir, 'Met500/Met500_mut_matrix.csv'), index_col=0)         ### 获取基因的突变情况以及基因的CNV情况
    ...
    ### 如果说最后的CNV数据进行了更详尽的划分(划分他是扩增还是减少这两种类型，那么最终的数据的输入类型就是下面的三种：突变、减少、扩增)，如果没有进行划分，那么就是上述的两种类型：['gene_final', 'cnv']
    if cnv_split:
        valid_cnv_ampl = valid_cnv_df.copy()
        valid_cnv_ampl[valid_cnv_ampl <= 0.0] = 0.
        valid_cnv_ampl[valid_cnv_ampl > 0.0] = 1.0
...
class TrainValidatePipeline:
    ...
    ### 将训练阶段以及测试阶段的预测结果等信息给保存到csv文件当中
    ...
    def run(self):
        ...
        x_train, y_train, info_train, cols_train = data.get_data()     ### 先获取要进行训练的数据

        data_types = cols_train.get_level_values(1).unique()       ### 读取患者他的基因属性数据，看看这里面的CNV有没有再进行详细的划分！
        if len(data_types) > 2:
            cnv_split = True
        else:
            cnv_split = False

        # divide the trainig set into two blanced training sets.(这两个数据集的数目是均衡的) Later we will train 2 models and combine their outputs
        ...
        info_train1 = np.concatenate((info_train_pos, info_train_neg1))        ### 保证当前的训练集中正负类别的样本都有且保持均衡
        ...
        # 3- New dataset ()
        ### 也就是说现在来获取测试阶段所用的验证数据集，就是两大类（原始状态和转移状态）
        ...
        if type(self.model_params) == list:
            for m in self.model_params:     ### 对于不同模型参数下各自训练一个模型
                # get model
                set_random_seeds(random_seed=20080808)

                ### 每套参数都是需要训练两个模型，最后将两个模型的结果进行综合来看！     也就是说原生性以及转移性这两种癌症分别作为单独的一类来进行训练！
                model1 = get_model(m)
                model2 = get_model(m)
                ...
                def predict(x_test, y_test, info_test, model_name, test_set_name):
                    pred = {}
                    y_pred_test2, y_pred_test_scores2 = self.predict(model2, x_test, y_test)         ### 测试预测的时候，同样的数据，两种模型都来预测一下得到各自的结果
                    y_pred_test1, y_pred_test_scores1 = self.predict(model1, x_test, y_test)

                    y_pred_test_scores = (y_pred_test_scores1 + y_pred_test_scores2) / 2.    ### 最终是两个模型的预测结果分数进行综合再除以2！
                    y_pred_test = y_pred_test_scores > 0.5

                    logging.info('scoring ...')
                    test_score = evalualte(y_test, y_pred_test, y_pred_test_scores)     ### 根据预测结果来获取测试分数以及构造混淆矩阵
                ...
                predict(x_test_primary, y_test_primary, info_test_primary, model_name, '_primary')         ### 分别来获取两种类型数据的预测结果并将其保存
    ...
    def save_layer_outputs(self, x_train_layer_outputs, y_train, y_train_pred, x_test_layer_outputs, y_test):
        ...
        for i, (x_train, x_test) in enumerate(zip(x_train_layer_outputs[:-2], x_test_layer_outputs[:-2])):
            ...
            pca = decomposition.PCA(n_components=50)      ### 使用PCA进行降维，其中这个n_components就是降维后的主成分的数量！
            # pca.fit(xx)
