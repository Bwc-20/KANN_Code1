...
class OneSplitPipeline:
    def __init__(self, task, data_params, pre_params, feature_params, model_params, pipeline_params, exp_name):
        ...
        self.task = task
        print("目前是在one_split.py文件中，看看此时的这个任务是谁！！", task)                     ### 目前这个task是classification_binary
        ...
        print("目前是在one_split.py文件中，看看此时的通道参数！", pipeline_params)
    ...
    def run(self):
        ...
        for data_params in self.data_params:
            ...
            # get data
            x_train, x_validate_, x_test_, y_train, y_validate_, y_test_, info_train, info_validate_, info_test_, cols = data.get_train_validate_test()          ### 就是来获取训练验证以及测试数据的！
            ...
            for m in self.model_params:
                ...
                model = get_model(model_params_)        ### 根据此时的模型参数来构造当前的这个模型
    ...
    def save_coef(self, model_list, cols):
        ...
        for model, model_params in model_list:
            ...
            logging.info('目前是在one_split.py 文件中， saving coef ')
            # print("目前是在one_split.py 文件中，当前读取的系数看是谁！", c_)          ## 现在这个c_就是那个重要性分数    在这里是有五个模型的，因为是五折交叉运算嘛，所以是训练了五个模型，每个模型他自己内部都包含了 self.coef_  这个是作为模型内部各个节点的重要性分数！，因此说是每个模型他都含有一个
            model_name_col = model_name
            if hasattr(model, 'get_named_coef') and c_ is not None:
                file_name = join(dir_name, 'coef_' + model_name)
                print("目前是在one_split.py 这个文件中的， 现在的这个文件名字是谁！", file_name)
                coef = model.get_named_coef()
                if type(coef) == list:
                    for i, c in enumerate(coef):
                        if type(c) == pd.DataFrame:
                            c.to_csv(file_name + '_layer' + str(i) + '.csv')
                            print("他应该在这里就保存了，这个文件名是！", file_name + '_layer' + str(i) + '.csv')
                elif type(coef) == dict:
                    for c in list(coef.keys()):
                        if type(coef[c]) == pd.DataFrame:
                            coef[c].to_csv(file_name + '_layer' + str(c) + '.csv')
                            print("目前是在one_split.py 这个文件中的，当前所保存的这个文件的名字是谁！", file_name + '_layer' + str(c) + '.csv')

            if type(c_) == list or type(c_) == tuple:
                coef_df[model_name_col] = c_[0]      ### 看来这个第一列的数据就是输入数据  基因型数据
                print("目前是在one_split.py 这个文件中的，这个重要性分数的首组应该是输入数据，基因型数据：", len(c_[0]), model_name_col)
            else:
                coef_df[model_name_col] = c_
        file_name = join(dir_name, 'coef.csv')
        print("目前是在one_split.py 这个文件中的，目前最终要进行保存的文件名是：", file_name)
    ...
    def predict(self, model, x_test, y_test, XiaoRongFlag=None):
    ...
    def preprocess(self, x_train, x_test):
        logging.info('现在是pipline/one_split.py文件，preprocessing....')
    ...
    def extract_features(self, x_train, x_test):
        if self.features_params == {}:
            return x_train, x_test
        logging.info('现在是pipline/one_split.py文件，feature extraction ....')
