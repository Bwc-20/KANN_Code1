...
def get_coef_from_model(model):
    coef = None
    ### 下面是来求这个模型中各层的节点的重要性分数
    if hasattr(model, 'coef_'):
        print("现在是在pipe_utils.py文件中，目前包不包含coef_")
    ...
    if hasattr(model, 'scores_'):
        print("现在是在pipe_utils.py文件中，目前包不包含scores_")
        coef = model.scores_

    if hasattr(model, 'feature_importances_'):
        print("现在是在pipe_utils.py文件中，目前包不包含feature_importances_")
        coef = np.abs(model.feature_importances_)
    print("现在是在pipe_utils.py文件中，目前是在这来保存模型的coef_结果的，目前的这个coef信息是：", coef)
    return coef
