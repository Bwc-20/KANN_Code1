...
def evalualte_classification_binary(y_test, y_pred, y_pred_score=None):
    print("现在是在utils.evaluate.py 这个文件中，现在看一下测试数据他的预测真实类别与预测类别情况（在这只返回两者的shape）", y_test.shape, y_pred.shape)
    accuracy = accuracy_score(y_test, y_pred)
    if y_pred_score is None:
        fpr, tpr, thresholds = metrics.roc_curve(y_test, y_pred, pos_label=1)
    else:
    ...
    recall = metrics.recall_score(y_test, y_pred)
    print("现在是在utils.evaluate.py 这个文件中，开始显示测试报告结果")
    logging.info(metrics.classification_report(y_test, y_pred))
    print("现在是在utils.evaluate.py 这个文件中，测试报告结果显示结束了！")
    from sklearn.metrics import average_precision_score
    aupr = average_precision_score(y_test, y_pred_score)
    score = {}
    score['accuracy'] = accuracy
