...
alpha = 0  # 正则化损失的权重    现在将他来作为整体的全局变量！


### 目前整个实验跑的时候用的是下面的这个函数！
...
def build_pnet2(optimizer, w_reg, w_reg_outcomes, add_unk_genes=True, sparse=True, loss_weights=1.0, dropout=0.5,
    ...
    # ### 注意！！注意！！ 现在这里通过修改sparse、sparse_first_layer这两个参数，来控制原来的P-Net模型变化为全连接的网络模型，看看会是怎么样的！
    # sparse, sparse_first_layer = False, False

    outcome, decision_outcomes, feature_n, RegLoss = get_pnet(ins,
                                                     features=features,
                                                     genes=genes,
    ...
    def custom_regularization_loss(alpha, RegLoss):
        scaled_loss = alpha * RegLoss  # 根据权重调整损失的强度
        def custom_loss(y_true, y_pred):
            # 原始的损失函数
            original_loss = K.binary_crossentropy(y_true, y_pred)
            # 定义最终的损失函数，添加正则化项
            loss_with_regularization = original_loss + scaled_loss
    ...
    if repeated_outcomes:
        ...
        print("测试一下这个决策输出集看看目前是不是走的这里！")                 ### 目前走的不是这，是上面！！
    ...
    print('目前在prostate_models.py文件中， loss_weights', loss_weights, n_outputs)
    ...
    model.compile(optimizer=optimizer,
                  loss=[custom_regularization_loss(alpha, RegLoss)] * n_outputs, metrics=[f1], loss_weights=loss_weights)       ### 根据优化器以及训练时的相关损失参数来进行拟合训练
    print("现在是修改之后的这个权重参数！！", alpha)
...
## 下面这个是先用PCA方法对神经网络的输出值进行降维，之后将降维后的结果用激活网络进行预测！
def build_PCAACT(optimizer, n_weights, w_reg, activation='tanh', loss='binary_crossentropy', data_params=None):
    print(data_params)
    ...
    layer1 = Dense(units=10, activation='relu', W_regularizer=l2(w_reg), name='h0')
    outcome = layer1(ins)

    # ### 下面这个为自己构造的标准的全连接神经网络情况！
    # # layer1 = Dense(9229, activation=activation, W_regularizer=l2(w_reg), name='h0')  ### 这个算是对应的那个基因层
    # # outcome = layer1(ins)
    # ### 下面的这个是自己手动进行添加的各层网络！
    ...
    # # outcome = Dense(1066, activation=activation_decision, W_regularizer=l2(w_reg), name='h2')(outcome)  ### 现在这个为第一个通路层网络！
    # # outcome = Dropout(0.7, name='dropout_2')(outcome)
    # # outcome = Dense(447, activation=activation_decision, W_regularizer=l2(w_reg), name='h3')(outcome)  ### 现在这个为第一个通路层网络！
    # # outcome = Dropout(0.8, name='dropout_3')(outcome)
    # outcome = Dense(147, activation=activation_decision, W_regularizer=l2(w_reg), name='h4')(ins)  ### 现在这个为第一个通路层网络！
    # outcome = Dropout(0.9, name='dropout_4')(outcome)
    # outcome = Dense(26, activation=activation_decision, W_regularizer=l2(w_reg), name='h5')(outcome)  ### 现在这个为第一个通路层网络！


    outcome = Dense(1, activation='sigmoid', name='output')(outcome)
    model = Model(input=[ins], output=outcome)
...
## 构建这个模型的输入和输出层的网络   现在这个构建的是传统的全连接神经网络
def build_dense(optimizer, n_weights, w_reg, activation='tanh', loss='binary_crossentropy', data_params=None):
    ...
    print(n)
    print("现在当前这一层神经元的数目是多少个！", n, float(n_features), float(n_weights))                  ### 这三个位置的取值分别为：3.0 27687.0 71009.0
