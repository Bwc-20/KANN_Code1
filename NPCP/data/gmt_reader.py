...
class GMT():
    # genes_cols : start reading genes from genes_col(default 1, it can be 2 e.g. if an information col is added after the pathway col)   开始从genes_col中读取基因（默认为1，可以是2，例如，如果在pathway col之后添加了一个信息col
    # pathway col is considered to be the first column (0)           Pathway col被认为是第一列
        ...
        with open(filename) as gmt:
            ...
            for row in data_list:
                ...
                for gene in genes[genes_col:]:
                    ...
                    dict = {'group': pathway, 'gene': gene}         ### 这个算是构造出通路与基因的对应字典！
                    data_dict_list.append(dict)

        df = pd.DataFrame(data_dict_list)
        # print df.head()

        return df      ### 最终返回出各个通路以及其各自所对应的基因的那个字典！
